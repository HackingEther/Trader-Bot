...
class TestStrategyEngine:
    ...
    def test_generates_intent_for_good_signal(self) -> None:
        ...
        intent = self.engine.evaluate(pred, Decimal("150.00"))
    ...
    def test_skips_existing_position(self) -> None:
        ...
        intent = self.engine.evaluate(pred, Decimal("150.00"), has_open_position=True)
        assert intent is None

    def test_short_direction(self) -> None:
        pred = _make_prediction(direction="short")
        intent = self.engine.evaluate(pred, Decimal("150.00"))
        assert intent is not None
