...
class TestTradableDataset:
    def test_builds_samples_from_bars(self) -> None:
        bars = generate_multi_symbol_bars(symbols=["AAPL", "MSFT"], count=200, seed=42)
        dataset = build_tradable_dataset(bars, min_history=30, max_hold_bars=30)
    ...
    def test_filter_label_matches_no_trade(self) -> None:
        bars = generate_multi_symbol_bars(symbols=["SPY"], count=200, seed=7)
        dataset = build_tradable_dataset(bars, min_history=20, max_hold_bars=20)
